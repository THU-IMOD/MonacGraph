...
/**
 * High-performance batch import client for LsmGraph (RocksDB-backed graph database)
...
public class BatchImportClient {
    ...
    private static void initializeSession(Client client) throws Exception {
        ...
        ResultSet results = client.submit(
                "graph = LsmGraph.open('batch_database'); " +
                        "g = graph.traversal(); " +
                        "'Session initialized'"
        );
        results.all().get();
     ...
     */
    private static void batchImportNodes(Client client) throws Exception {
