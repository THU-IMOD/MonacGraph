 ...
 */
public class LsmEdge extends LsmElement implements Edge {

    // Prefix for edge keys in RocksDB storage
    public static final byte[] EDGE_PREFIX = "EDGE:".getBytes();

    private final LsmVertex outVertex;  // Outgoing vertex (source)
    private final LsmVertex inVertex;   // Incoming vertex (target)
     ...
     */
    public LsmEdge(final LsmGraph graph, final Object id, final String label,
                   final LsmVertex outVertex, final LsmVertex inVertex, final Object... keyValues) {
        ...
        for (int i = 0; i < cleaned.length; i += 2) {
            ...
            this.properties.put(key, new LsmProperty<>(this, key, value, graph));
        }
     ...
     */
    public LsmEdge(final LsmGraph graph, long edgeHandle) {
        super("", "edge", graph);
        ...
        try (ByteArrayInputStream bais = new ByteArrayInputStream(data);
            ...
            this.outVertex = (LsmVertex) graph.vertex(outVertexId);
            this.inVertex = (LsmVertex) graph.vertex(inVertexId);

            // Initialize properties from deserialized data
            this.properties = new HashMap<>();
            for (Map.Entry<String, Object> entry : props.entrySet()) {
                this.properties.put(entry.getKey(), new LsmProperty<>(this, entry.getKey(), entry.getValue(), graph));
            }
        } catch (IOException e) {
     ...
     */
    private static Object generateId(final LsmVertex outVertex, final String label, final LsmVertex inVertex) {
    ...
    public <V> Property<V> property(String key, V value) {
        ...
        final Property<V> newProp = new LsmProperty<>(this, key, value, graph);
